...
function buildQuery(className, rawCSS) {
  ...
  const innerContent = output.substring(
    output.indexOf("{") + 1,
    output.lastIndexOf("}")
  );
  output = `
...
function buildCSS(className, rawCSS) {
  ...
  const parsePseudos = () => {
    ...
    const pseudo = output.substring(
      start,
      findRightEndBracketPosition(output, start) + 1
    );
...
export function renderCSS() {
  let renderedCSS = "";
  Object.keys(docCSS).forEach(
    (classHash) => (renderedCSS += docCSS[classHash].rendered)
  );
...
export const setTheme = (styled.setTheme = (selectedTheme) =>
  (theme = Object.assign({}, selectedTheme)));
export const css = (styled.css = (strings, ...keys) =>
  buildAndRenderCSS(strings, keys, { theme }));
